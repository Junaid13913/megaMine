...
/* Metric cards */
.metrics-grid{display:flex;flex-wrap:wrap;gap:1px;
              border:1px solid var(--slate-200);margin-bottom:20px;
              background:var(--slate-200)}
.metric-card{background:var(--white);padding:12px 16px;
             text-align:center;flex:1;min-width:110px}
.metric-value{font-size:20px;font-weight:700;color:var(--navy-900);
...
def _header(title, run_info):
  ...
  <div class="report-header-grid">
    <div>
      ...
      <div class="report-subtitle">Generated by {_h(version)} &middot; APML, Ajou University &middot; {_h(generated)}</div>
    </div>
    <div class="report-meta">
      <div><span>Query</span>{_h(query[:80])}{'...' if len(query)>80 else ''}</div>
