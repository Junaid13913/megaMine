...
def _build_summary(rows_all, rows_ver, trend_ver, contra_ver,
    ...
    interp = f"""
<div class="subsection" style="border-left:4px solid #2563eb;background:#eff6ff;max-width:900px;">
  <h3 class="subsection-title" id="interp-summary">📝 Interpretation Summary</h3>
  <p style="font-size:0.82rem;line-height:1.65;color:#1e293b;max-width:860px;">
  ...
  <br><br>
  ⚠️ <strong>ClinicalTrials results shown as ≥200 are capped</strong> and may include off-target trials.
  ...
  gene–drug–alteration–cancer–direction combinations.
  ...
  673 rows ≠ 673 independent studies.
    ...
    cards = '<div class="metric-grid">\n' + "\n".join([
        ...
        _metric_card("Clean Cancer", f"{n_clean:,}", "🏷️", "#00a8a8"),
        ...
        _metric_card("Canonical Types", f"{n_canon:,}", "🏷️", "#2ea87e"),
        ...
        _metric_card("Contradiction Flags", f"{n_flag:,}", "⚠️", "#e8a020"),
        _metric_card("Trial Pairs", f"{n_trials:,}", "🏥", "#00a8a8"),
        _metric_card("Graph Nodes", f"{n_nodes:,}", "🕸️", "#1e2d3d"),
    ...
    if n_raw > 0 and n_canon > 0:
        ...
        qc.append(_qc_badge(lvl, f"Cancer normalization: {n_raw} raw labels → {n_canon} canonical types (ratio {ratio:.1f}x)"))
    if n_rejected > 0:
    ...
    if n_ro > 0 and n_re > 0:
        ...
        qc.append(_qc_badge("warn" if ratio > 5 else "ok", f"Resistance refinement: {n_ro} broad → {n_re} direct evidence ({ratio:.0f}x reduction)"))
    if n_all > 0:
        pct = n_ver / n_all * 100
        qc.append(_qc_badge("fail" if pct < 10 else ("warn" if pct < 30 else "ok"), f"Verification rate: {n_ver}/{n_all} ({pct:.1f}%)"))
    if n_cn > 15:
        qc.append(_qc_badge("warn", f"High cancer graph nodes ({n_cn}) — check canonical_cancer_type"))
    else:
    ...
    try:
        if rows_ver is not None:
            ...
            # Use exact canonical match — substring matching is imprecise
...
<div class="subsection" style="border-left:4px solid #0d9488;background:#f0fdfa;max-width:100%;box-sizing:border-box;">
  ...
  <div style="display:flex;gap:16px;flex-wrap:wrap;margin-top:10px;max-width:600px;">
  ...
  <p style="font-size:0.78rem;color:#6b7280;margin-top:10px;line-height:1.6;max-width:700px;">
...
def _build_resistance(rows_all, rows_ver, max_rows):
    ...
    try:
        if df is not None:
            ...
            fig.update_layout(title="Resistance Refinement: Broad → Precise", yaxis_title="Count")
            parts.append(_subsection("Resistance Refinement", _plot_html(fig),
                "resistance_evidence captures only direct statements — prevents overcounting."))
    except Exception as e:
    ...
    try:
        ...
        if dv is not None and "resistance_evidence" in dv.columns and "drug_primary" in dv.columns:
            ...
            _pb = _subsection("Resistance by Drug", _plot_html(fig3, 320),
                "Literature-derived — does not imply clinical failure.")
        else:
            _pb = ""
    except Exception as e:
...
def _build_matrix(rows_ver):
    ...
    try:
        ...
        fig = px.imshow(mat, color_continuous_scale="Blues",
                        title="Gene × Drug Evidence Count (Verified)",
        ...
        if "resistance_evidence" in sub.columns:
            ...
            if len(sub_r) > 0:
                ...
                fig2 = px.imshow(mat_r, color_continuous_scale="Reds",
                                 title="Gene × Drug Resistance Evidence",
                                 labels=dict(x="Drug", y="Gene", color="Count"))
    ...
    return _section("Gene–Drug Evidence Matrix", "".join(parts), "matrix")
...
def _build_temporal(trend_ver, profile_ver):
    ...
    # A. Trend category distribution — horizontal bar, color coded
    try:
    ...
    # B. Top triplets ranked by paper count — horizontal bar
    try:
    ...
    # C. SINGLE MULTI-LINE PLOT — all triplets in one chart
    # Each triplet gets its own color pair (efficacy=solid, resistance=dashed)
    # No subplots — no overlap — clean and readable
    try:
        if profile_ver is not None and len(profile_ver) > 0:
            ...
            if yb and ep and rp and "biomarker" in profile_ver.columns:
                ...
                # Color palette — one color per triplet
                PALETTE = [
                    "#5b4fcf","#f05a28","#00a8a8",
                    "#c0398e","#e8a020","#2ea87e",
                ]

                # ── Plot 1: Efficacy paper counts — all triplets ──
                fig_eff = go.Figure()
                ...
                fig_eff.update_layout(
                    title="Efficacy Papers Over Time — Top Gene-Drug Pairs",
                ...
                # ── Plot 2: Resistance paper counts — all triplets ──
                fig_res = go.Figure()
                for idx, (_, trow) in enumerate(top6.iterrows()):
                    mask = ((profile_ver["biomarker"] == trow["biomarker"]) &
                            (profile_ver["drug_primary"] == trow["drug_primary"]))
                ...
                fig_res.update_layout(
                    title="Resistance Papers Over Time — Top Gene-Drug Pairs",
                    xaxis_title="Year Bin",
    ...
    # D. Resistance emergence heatmap — all triplets × year bins
    try:
        if profile_ver is not None and len(profile_ver) > 0:
            yb = _safe_col(profile_ver, "year_bin")
            rp = _safe_col(profile_ver, "resistance_papers")
            if yb and rp and "biomarker" in profile_ver.columns:
                ...
                parts.append(_subsection(
                    "Resistance Evidence Heatmap (Triplets × Year Bins)",
                    _plot_html(fig4, 500),
    ...
    try:
        if trend_ver is not None and len(trend_ver) > 0:
            # Show shares only in main table — ratios in download only
            # Shares sum to 1.0 — directly interpretable
            ...
            parts.append(_subsection("Temporal Summary Table",
                ...
                "Shares are directional fractions summing to 1.0 — more interpretable than ratios. "
...
def _build_trials(trials_df):
    ...
    try:
        ...
        parts.append(_subsection("Trials Summary Table",
            _table_html(tbl),
            "≥200 = result capped at API maximum — includes off-target trials. "
...
def _build_graph_section(nodes_df, edges_df, rows_verified=None):
    ...
    try:
        if "edge_type" in edges_df.columns:
            ...
            _pb4 = _subsection("Edge Types", _plot_html(fig2, 320),
                "ASSOCIATED_WITH and EVALUATED_IN: conservative labels — literature mining does not prove expression or treatment.")
        else:
            _pb4 = ""
    except Exception as e:
        ...
        if "node_type" in nodes_df.columns:
            ...
            if source_col and target_col:
                ...
                fig_m = px.imshow(matrix,
                    ...
                    title="Edge Source→Target Node Type Matrix",
                    labels=dict(x="Target Node Type", y="Source Node Type", color="Edge Count"),
                    text_auto=True,
                )
                parts.append(_subsection("Source→Target Node Type Heatmap",
    ...
    # Evidence quality ranking — meaningful scoring not degree
    try:
        if "node_type" in nodes_df.columns:
            ...
            if len(ev_nodes) > 0:
                ...
                # Clean up numeric columns — remove .0 suffix
                ...
                # No merge needed — biomarker/drug/cancer/pmid already in nodes CSV
                ...
                score_note = (
                    ...
                    "+ verifier_confidence×1.5 + recency_bonus (2022+=1.0) "
                    "+ resistance_bonus - contradiction_penalty. "
                    "This is a literature prioritization score — NOT a clinical actionability score."
                )
    ...
    # Show only key node types in main report — full data in CSV
    try:
        ...
        if len(cancer_nodes_t) > 0:
            # Shorten raw_cancer_type — only show first 60 chars
        ...
        # Edge type summary only — not full edge table
        if "edge_type" in edges_df.columns:
